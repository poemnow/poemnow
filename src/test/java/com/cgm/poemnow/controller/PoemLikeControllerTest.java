...
class PoemLikeControllerTest {
    ...
    @Autowired
    PoemLikeController poemLikeController;

    @Test
    @DisplayName("시 좋아요 추가 및 리스트 조회 테스트")
    void poemLikeAddAndpoemLikeList() throws Exception {
        // given 데이터를 보내줄 애를 만들자
        //Follow 객체를 넣으니 만들자
        PoemLike newPoemLike = PoemLike.builder()
                .userId(1)
                .poemId(9)
                .build();
        //그 객체 제이슨으로 바꾸자
        String jsonData = new Gson().toJson(newPoemLike);

        mvc.perform(post("http://127.0.0.1:8080/like/poemLikeAdd")
        ...
        //내가 팔로우 하는 사람 조회 테스트
        String userId="1";
        MvcResult mvcResult = mvc.perform(get("http://127.0.0.1:8080/like/likePoemList")
                        .contentType(MediaType.APPLICATION_JSON) //타입설정
                        .content(userId))
        ...
        for (JsonElement jsonElement : jsonArray) {
            PoemLike likes = gson.fromJson(jsonElement, PoemLike.class);
            if (8 == (likes.getPoemId())) {
                isContained = true;
                break;
            }
        }
    ...
    @Test
    @DisplayName("시 좋아요 삭제 및 데이터 개수 변화 확인 테스트")
    void poemLikeRemoveTest() throws Exception {

        // 전체조회하여 첫번째 데이터를 꺼내옵니다.
        MvcResult mvcResult1 = mvc.perform(get("/like/likePoemList"))
                .andExpect(status().isOk())
                .andDo(print())
                .andReturn();
        ...
        int id = jsonArray.get(0).getAsJsonObject().get("poemId").getAsInt();
        int size = jsonArray.size();

        // 첫번째 데이터를 삭제합니다.
        mvc.perform(delete("/like/poemLikeRemove?poemId="+id))
                .andExpect(status().isOk());

        MvcResult mvcResult2 = mvc.perform(get("/like/likePoemList"))
                .andExpect(status().isOk())
                .andDo(print())
                .andReturn();
